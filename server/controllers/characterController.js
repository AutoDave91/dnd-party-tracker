...
const characters = [{campaign: 'TOA', player: 'AutoDave', name: 'Nortle', class: 'Druid', role: 'Healer', lvl: 1, max_hp: 10, ac: 19, current_hp: 10, temp_hp: 0, str: 12, dex: 12, con: 14, int: 13, wis: 16, cha: 8, health: 'ok'},
...
function editCharacter(req, res){
    let party = characters.filter(character => character.campaign === req.body.campaign)
    let adventurer = party.filter(character => character.name ===req.body.name)
    if(req.body.effect === level){
        console.log('leveled up')
    } else if (req.body.effect === damage){
        console.log('Ouch')
    } else if (req.body.effect === heal){
        console.log('That feels better')
    } else if (req.body.effect === temp_gain){
        console.log('That might help')
    } else if (req.body.effect === temp_loss){
        console.log('Oh dear...')
    }
    party = characters.filter(character => character.campaign === req.body.campaign)
    res.json(party)
}
