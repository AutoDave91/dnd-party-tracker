...
{campaign: 'Winds of Change', player: 'AutoDave', name: 'Gnaagark', class: 'Barb (Eagle Totem)', role: 'Tank', lvl: 3, max_hp: 25, ac: 15, current_hp: 25, temp_hp: 0, str: 17, dex: 14, con: 16, int: 10, wis: 8, cha: 8, health: 'ok'},
...
async function getParty(req, res){
    ...
    for(let i = 0; i < party.length-1; i++){
        let half = party[i].max_hp / 2
        if(party[i].current_hp === 0){
            party[i].health = 'downed'
        } else if (party[i].current_hp > half){
            party[i].health = 'ok'
        } else if(party[i].current_hp < half){
            party[i].health = 'half'
        }
